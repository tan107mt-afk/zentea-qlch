...
async function loadViolations(){
  ...
  try {
    const d = await branchDbGet('violations');
    if(d) violationsData = d;
    else {
...
async function saveViolations(){
  try { await branchDbSet('violations', violationsData); } catch(e){}
...
async function loadCustomRecipes(){
  customRecipes = [];
  try {
    const d = await branchDbGet('custom-recipes');
    if(d) customRecipes = d;
    else {
      const s = localStorage.getItem(branchKey('zentea-custom-recipes'));
      if(s) customRecipes = JSON.parse(s);
    }
